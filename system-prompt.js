...
const TIER_CONFIGS = {
  ...
  decision_friction_review: { limit: 1, monthly: false, name: 'Decision Friction Review', price: '$149', stripeUrl: 'https://strategic-flow-pro.replit.app/decision-friction-review/' },
  ...
  architecture: { limit: Infinity, monthly: true,  name: 'Architecture',    price: 'Suite',   stripeUrl: null }
};
...
A/B SUBJECT LINES: In "ab_subjects", generate 3 subject line variants — one per angle: curiosity_gap (creates information tension — reader must open to resolve it), specific_outcome (names the exact measurable result), pattern_interrupt (breaks genre convention — unexpected enough to stop scrolling). Never include metrics unless they appear verbatim in the source. Each variant: subject, angle, reasoning, predicted_lift.

AUDIENCE SEGMENTS: In "segments", identify the 3 audience segments this email converts best with. Consider job title, company stage, pain intensity, purchase intent, awareness level. Each: name, description, why_converts, behavioral_signal.

FOLLOW-UP CALENDAR: In "follow_ups", suggest 3 sequential follow-up email topics that build momentum from this newsletter. Each must target a different angle or pain point, have a specific subject line, and be timed sequentially. Each: topic, subject_line, angle, timing, why_now.
...
${emailType === 'event_announcement' ? `TIMELINE COHESION CHECK: In "cohesion", score your REBUILT timeline email using these 5 criteria (0-2 each, total 0-10):
1. CARD COUNT (0-2): Are there between 3 and 8 featureCards, one per distinct date/milestone/phase? 2=yes, 1=slightly off, 0=collapsed into fewer or missing
2. CARD SPECIFICITY (0-2): Does each card title name a real date, deadline, or phase label from the source? 2=all specific, 1=some vague, 0=generic or invented
3. CHRONOLOGICAL ORDER (0-2): Are cards ordered chronologically as they appear in the source? 2=yes, 1=minor reordering, 0=shuffled or missing order
4. CTA RESOLUTION (0-2): Does the ctaText direct the reader toward the earliest actionable deadline or registration? 2=direct, 1=related, 0=generic
5. NO NARRATIVE COLLAPSE (0-2): Did you avoid collapsing timeline milestones into narrative body paragraphs? 2=fully timeline, 1=partial narrative leakage, 0=returned body[] instead of featureCards
TARGET: 8+/10. If total is below 7, populate "rewrite_instructions" with specific fixes — do NOT suggest converting to narrative paragraphs.
Fields: cohesion_score (number 0-10), criteria ({hook_specificity, solution_connection, consequence_quality, cta_resolution, narrative_unity} each 0-2), narrative_breaks (array of {location, issue, fix} — empty if none), cta_earned (boolean), overall_verdict (1 sentence), rewrite_instructions (string with specific rewrites if score < 7, otherwise null).` : `COHESION CHECK: In "cohesion", score your REBUILT email using these 5 criteria (0-2 each, total 0-10):
